"""
Old module used to extract indoor calibration information
"""
...
CALIBRATION_INDOOR_LABELS = ["WACOM Software - Measurement File",
           "Sample details",
...
CALIBRATION_INDOOR_FILES_FOLDER = os.path.join(os.path.dirname(os.getcwd())+r"/assets/calibration_indoor_files/")




def _get_calibration_indoor_labels_position(files_folder = CALIBRATION_INDOOR_FILES_FOLDER,
                                     labels = CALIBRATION_INDOOR_LABELS):
    """
    ...
    """
    rows_list = []
    filenames = [f for f in os.listdir(files_folder) if f.endswith(r".txt") and f.startswith(r"WAC")]
    info_types = ('filename', 'info_line', 'info_value', 'start_line', 'sample_line', 'end_line')
    for filename in filenames:
        fhand = open(files_folder + filename)
        sample_ck, info_value_previous = False, ""
        for index, line in enumerate(fhand):
            value = line.rstrip()
            if value != "":
                #transforming into tuple to avoid error ?
                if value.startswith(tuple(labels)):
                    if sample_ck == True:
                        sections_tmp = {
                            info_types[0]: filename,
                    ...
                    info_line = str(index)
                    info_value = str([i for i in list(CALIBRATION_INDOOR_LABELS) if (i in value)][0])
                    # Dealing with old format (2x "Meas Results")
                    if (info_value == info_value_previous) and (
                            info_value_previous == "Pyranometer Calibration Reference Meas Results"):
...
def _main_info_to_dataframe(files_folder = CALIBRATION_INDOOR_FILES_FOLDER, 
                            labels_position: pd.DataFrame = pd.DataFrame(),
    ...
    #17/11/20 overlaps with other get_calibration_indoor_labels_position function
    filenames = [f for f in os.listdir(files_folder) if f.endswith(r".txt") and f.startswith(r"WAC")]

    if len(labels_position) == 0:
        labels_position = _get_calibration_indoor_labels_position(files_folder=files_folder,
                                           labels=CALIBRATION_INDOOR_LABELS)
    for fname in filenames:
        file_path = str(files_folder) + str(fname)
        file_sections_positions = labels_position[(labels_position.filename == fname)]
        if file_sections_positions.shape[0] == 0:
            print(file_path + " not found")
        elif (file_sections_positions.shape[0] != 0):
            # extracting indexes for Sample details:
            id_info = CALIBRATION_INDOOR_LABELS[1]
            sample_id_index = int(
                file_sections_positions[(file_sections_positions.info_value == id_info)]["start_line"].values)
            sample_type_index = int(
                file_sections_positions[(file_sections_positions.info_value == id_info)]["start_line"].values) + 3
            sample_details_index = int(file_sections_positions[(file_sections_positions.info_value == id_info)][
                                           "start_line"].values) + 6  # 120620 added to look for maintenance
            # extracting index for Reference Pyranometer Details
            id_info = CALIBRATION_INDOOR_LABELS[2]
            ref_id_index = int(
                file_sections_positions[(file_sections_positions.info_value == id_info)]["start_line"].values)
            ref_type_index = int(
                file_sections_positions[(file_sections_positions.info_value == id_info)]["start_line"].values) + 2
            ref_sensitivity_factor_index = int(
                file_sections_positions[(file_sections_positions.info_value == id_info)]["start_line"].values) + 4
            # extracting index for "Wacom Lamp Calibration Record Data
            id_info = CALIBRATION_INDOOR_LABELS[6]
            ...
            id_info = CALIBRATION_INDOOR_LABELS[7]
            # 2 values per row
            responses_index = int(
                file_sections_positions[(file_sections_positions.info_value == id_info)]["start_line"].values)
            irradiances_index = int(
                file_sections_positions[(file_sections_positions.info_value == id_info)]["start_line"].values) + 1
            # row list
...
def _submeasures_to_dataframe(labels_position, files_folder, files_list,
...
def _results_to_dataframe(labels_position, files_folder, files_list,
    ...
    for fname in files_list:
        ...
        if file_sections_positions.shape[0] == 0:
            ...
            ref_start_index = int(
                file_sections_positions[(file_sections_positions.info_value == id_info)]["start_line"].values) - 1
            ref_end_index = int(
                file_sections_positions[(file_sections_positions.info_value == id_info)]["end_line"].values)
